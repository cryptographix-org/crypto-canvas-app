...
export class NodeElement extends NodeInfo {
  ...
  renderButton() {
    ...
    var nodeButtonGroup = this.nodeSelector.append("svg:g")
    ...
    nodeButtonGroup.append("rect")
      ...
      .attr("fill", "#eee");//function(d:NodeInfo) { return this._def.color;})
    ...
    nodeButtonGroup.append("rect")
      ...
      .attr("fill", () => { return this._def.color; })
      .attr("cursor", "pointer")
      .on("mousedown", () => { if (!state.inMode(CanvasMode.LASSOING) && this.isButtonEnabled()) { canvas.focusView(); this.nodeSelector.attr("fill-opacity", 0.2); d3.event.preventDefault(); d3.event.stopPropagation(); } })
      .on("mouseup", () => { if (!state.inMode(CanvasMode.LASSOING) && this.isButtonEnabled()) { this.nodeSelector.attr("fill-opacity", 0.4); d3.event.preventDefault(); d3.event.stopPropagation(); } })
      .on("mouseover", () => { if (!state.inMode(CanvasMode.LASSOING) && this.isButtonEnabled()) { this.nodeSelector.attr("fill-opacity", 0.4); } })
      .on("mouseout", () => {
        if (!state.inMode(CanvasMode.LASSOING) && this.isButtonEnabled()) {
          ...
          this.nodeSelector.attr("fill-opacity", op);
        }
      })
  ...
  **/
  renderMain() {
    let canvas = this.canvas;
    let state = canvas.state;
    let nodeSelector = this.nodeSelector;
    ...
    var mainRect = nodeSelector.append("rect")
      ...
      .on("touchend", (d: NodeInfo) => {
        canvas.touchEnd();
        this.nodeMouseUp();
      })
      .on("mouseover", () => {
        if (state.inMode(CanvasMode.IDLE)) {
          var node = this.nodeSelector;
  ...
  renderIcon() {
    let info: NodeInfo = this;

    var icon_group = this.nodeSelector.append("g")
    ...
    this.updateIcon(this.nodeSelector);
  ...
  updateOutputPorts(nodeSelector: NodeSelector) {
    ...
    let _ports: PortSelector = nodeSelector.selectAll<SVGElement, number>(".port_output").data(this.ports);
  ...
  constructor(el: SVGElement, public canvas: CanvasElement, info: NodeInfo) {
    super(info);

    let nodeSelector = this.nodeSelector = d3.select<SVGElement, NodeElement>(el);

    var isLink = (this.type === "link in" || info.type === "link out");

    nodeSelector.attr("id", info.id);
    var l = this.description; //REthis.utils.getNodeLabel(d:NodeInfo);
    ...
    /*      if (this._def.badge) {
            ...
            if (this._def.onbadgeclick) {
              badgeRect.attr("cursor", "pointer")
                .on("click", function(d:NodeInfo) { this._def.onbadgeclick.call(d:NodeInfo); d3.event.preventDefault(); });
            }
          }*/
    ...
    **/
    ...
    if (!isLink) {
      ...
      if (info._def.align) {
        text.attr("class", "node_label node_label_" + info._def.align);
        if (info._def.align === "right") {
  ...
  updateElement(thisElement: SVGElement) {
    ...
    if (this.dirty) {
      ...
      //if (this.x < -50) deleteSelection();  // Delete nodes if dragged back to palette
      ...
      if (!this.canvas.state.inMode(CanvasMode.MOVING_ACTIVE)) {
        ...
        nodeSelector.selectAll(".node_icon")
          .attr("y", () => { return (this.h - +this.nodeSelector.attr("height")) / 2; });
        ...
        nodeSelector.selectAll(".node_right_button")
          ...
          .attr("fill", () => {
            return typeof this._def.button.color === "function" ? this._def.button.color.call(this) : (this._def.button.color != null ? this._def.button.color : this._def.color)
          });
  ...
  portMouseUp(type, index) {
    ...
    if (state.inMode(CanvasMode.JOINING) || state.inMode(CanvasMode.QUICK_JOINING)) {
      let mouseUpNode: NodeElement = this;

      // When we DROP link on OTHER port
      ...
      /*
      for (i = 0; i < state.dragLinks.length; i++) {
        if (portType != state.dragLinks[i].portType && mouseup_node !== state.dragLinks[i].node) {
